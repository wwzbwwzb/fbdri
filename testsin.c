...
void draw_thing(viewport_t *viewport)
{
	...
	{
		vector_t *vertex1 = &animation->vertices[1][j + 1];
		vector_t *vertex2 = &animation->vertices[0][j + 1];
		vector_t *vertex3 = &animation->vertices[0][j];
		vector_t *vertex4 = &animation->vertices[1][j];
		vector_t *normal = &animation->normals[0][j];
		vector_t *texcoord1 = &viewport->tex_coords[1][j + 1];
		vector_t *texcoord2 = &viewport->tex_coords[0][j + 1];
		vector_t *texcoord3 = &viewport->tex_coords[0][j];
		vector_t *texcoord4 = &viewport->tex_coords[1][j];
...
// Get vertices
	for(i = 0; i < viewport->total_frames; i++)
	{
		...
		{
			vector_t *vertex1 = &animation->vertices[0][k];
			vector_t *vertex2 = &animation->vertices[1][k];

			vertex1->z = vertex2->z = 
				CUBE_DEPTH * 
				sin(((float)k + (float)i / viewport->total_frames * M_PI) * 1);
			vertex1->x = vertex2->x = (float)k / RESOLUTION * CUBE_WIDTH - CUBE_WIDTH / 2;
			vertex1->y = -CUBE_HEIGHT / 2;
			vertex2->y = CUBE_HEIGHT / 2;
		}
	}
...
// Get normals
	for(i = 0; i < viewport->total_frames; i++)
	{
		...
		for(k = 0; k < RESOLUTION - 1; k++)
		{
			vector_t *vertex1 = &animation->vertices[0][k];
			vector_t *vertex2 = &animation->vertices[1][k];
			vector_t *vertex3 = &animation->vertices[0][k + 1];
			vector_t *out = &animation->normals[0][k];
...
// Get texture coords
	for(k = 0; k < RESOLUTION; k++)
	{
		vector_t *out1 = &viewport->tex_coords[0][k];
		vector_t *out2 = &viewport->tex_coords[0][k];

		out1->x = out2->x = (float)k / RESOLUTION * MOVIE_W / TEXTURE_SIDE;
		out2->y = 0.0;
		out1->y = (float)MOVIE_H / TEXTURE_SIDE;
	}
}

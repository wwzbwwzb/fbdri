	...
	for(i = 0; i < viewport->total_frames; i++)
	{
		...
		for(j = 0; j < RESOLUTION; j++)
		{
			for(k = 0; k < RESOLUTION; k++)
			{
				...
				vertex->x = (float)k / RESOLUTION * CUBE_WIDTH - CUBE_WIDTH / 2;
				vertex->y = (float)j / RESOLUTION * CUBE_HEIGHT - CUBE_HEIGHT / 2;
			}
		}
	}
...
// Get texture coords
	for(j = 0; j < RESOLUTION; j++)
	{
		for(k = 0; k < RESOLUTION; k++)
		{
			...
			out->x = (float)k / RESOLUTION * MOVIE_W / TEXTURE_SIDE;
			out->y = (float)(RESOLUTION - j) / RESOLUTION * MOVIE_H / TEXTURE_SIDE;
		}
	}
}

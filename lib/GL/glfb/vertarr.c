...
static GLuint __glFBTypeSize(GLenum enm)
{
    switch (enm) {
...
{
    __GLFBcontext *gc = __glFBGetCurrentContext();
    __GLFBvertexArrayPointerState *normalPointer = &gc->state.vertArray.normal;
    ...
    /* Set internal state */
    if (stride == 0) {
	normalPointer->skip = 3 * __glFBTypeSize(type);
    } else {
...
{
    __GLFBcontext *gc = __glFBGetCurrentContext();
    __GLFBvertexArrayPointerState *colorPointer = &gc->state.vertArray.color;
    ...
    /* Set internal state */
    if (stride == 0) {
        colorPointer->skip = size * __glFBTypeSize(type);
    } else {
...
{
    __GLFBcontext *gc = __glFBGetCurrentContext();
    __GLFBvertexArrayPointerState *indexPointer = &gc->state.vertArray.index;
    ...
    /* Set internal state */
    if (stride == 0) {
	indexPointer->skip = __glFBTypeSize(type);
    } else {
...
{
    __GLFBcontext *gc = __glFBGetCurrentContext();
    __GLFBvertexArrayPointerState *texCoordPointer =
    ...
    /* Set internal state */
    if (stride == 0) {
	texCoordPointer->skip = __glFBTypeSize(type) * size;
    } else {
...
{
    __GLFBcontext *gc = __glFBGetCurrentContext();
    __GLFBvertexArrayPointerState *edgeFlagPointer = &gc->state.vertArray.edgeFlag;
...
{
    __GLFBcontext *gc = __glFBGetCurrentContext();
    __GLFBvertArrayState *va = &gc->state.vertArray;
    GLint j;
    ...
    for (j=0; j<__GLFB_MAX_TEXTURE_UNITS; ++j) {
...
void glDrawArrays(GLenum mode, GLint first, GLsizei count)
{
    ...
    const GLubyte *vaPtr = NULL, *naPtr = NULL, *caPtr = NULL, 
                  *iaPtr = NULL, *tcaPtr[__GLFB_MAX_TEXTURE_UNITS];
...
void glClientActiveTextureARB(GLenum texture)
{
    ...
    if (unit < 0 || __GLFB_MAX_TEXTURE_UNITS <= unit) {
	__glFBSetError(gc, GL_INVALID_ENUM);
	return;
    }
...
void __glFBInitVertexArrayState(__GLFBcontext *gc)
{
    __GLFBvertArrayState *va = &gc->state.vertArray;
    GLint i;
    ...
    for (i=0; i<__GLFB_MAX_TEXTURE_UNITS; ++i) {
        __GLFBvertexArrayPointerState *texCoord = &va->texCoord[i];

...
*/
static void FillBitmap(__GLFBcontext *gc, GLint width, GLint height,
...
** ALIGNMENT = 1.
*/
void __glFillImage(__GLFBcontext *gc, GLint dim, GLint width, GLint height,
		   ...
		   const GLvoid *userdata, GLubyte *newimage, GLubyte *modes)
{
	...
	if (swapBytes) {
		...
		if (rowSize == elementsPerRow * elementSize) {
		    ...
		    __GLFB_MEM_COPY(iter2, itera,
				   elementsPerRow * elementSize * height);
		    iter2 += elementsPerRow * elementSize * height;
		} else {
		    iter = itera;
		    for (i = 0; i < height; i++) {
			__GLFB_MEM_COPY(iter2, iter, elementsPerRow*elementSize);
	...
	if (dim == 3) {
	    GLubyte *pc = modes;
	    __GLFB_PUT_CHAR(0,GL_FALSE);
	    __GLFB_PUT_CHAR(1,GL_FALSE);
	    __GLFB_PUT_CHAR(2,0);
	    __GLFB_PUT_CHAR(3,0);
	    __GLFB_PUT_LONG(4,0);
	    __GLFB_PUT_LONG(8,0);
	    __GLFB_PUT_LONG(12,0);
	    __GLFB_PUT_LONG(16,0);
	    __GLFB_PUT_LONG(20,0);
	    __GLFB_PUT_LONG(24,0);
	    __GLFB_PUT_LONG(28,0);
	    __GLFB_PUT_LONG(32,1);
	} else {
	    GLubyte *pc = modes;
	    __GLFB_PUT_CHAR(0,GL_FALSE);
	    __GLFB_PUT_CHAR(1,GL_FALSE);
...
*/
static void EmptyBitmap(__GLFBcontext *gc, GLint width, GLint height,
...
*/
/* ARGSUSED */
void __glEmptyImage(__GLFBcontext *gc, GLint dim, GLint width, GLint height, 
		    GLint depth, GLenum format, GLenum type,
		    const GLubyte *sourceImage, GLvoid *userdata)
{
	...
	for (h = 0; h < depth; h++) {
		...
		/* Ha!  This is mondo easy! */
		__GLFB_MEM_COPY(itera, sourceImage,
			       elementsPerRow * elementSize * height);
		sourceImage += elementsPerRow * elementSize * height;
	    } else {
		iter = itera;
		for (i = 0; i < height; i++) {
		    __GLFB_MEM_COPY(iter, sourceImage,

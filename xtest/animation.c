...
#define CUBE_DEPTH (float)1
#define FREQUENCY 1.0
// Should be an integer multiple of the cube dimensions
#define XRESOLUTION 256      // Equilateral triangles require 2 * w
	...
	for(i = 0; i < 3; i++)
	{
		...
		switch(viewport->function)
		{
			...
			case 1:
				...
				break;
			case 2:
				triangle->point[i].z = sin(triangle->point[i].x * viewport->frequency +
						2.0 * M_PI - (float)frame / viewport->total_frames * 2 * M_PI) * 
					(float)viewport->amplitude;
				break;
			case 3:
				triangle->point[i].z = sin(triangle->point[i].y * viewport->frequency +
						2.0 * M_PI - (float)frame / viewport->total_frames * 2 * M_PI) * 
					(float)viewport->amplitude;
				break;
		}
...
static void init_animation(viewport_t *viewport)
{
	...
	if(!viewport->animation)
		viewport->animation = calloc(1, sizeof(animation_frame_t) * viewport->total_frames);
...
void texture_iterate(viewport_t *viewport)
{
	...
	for(i = 0; i < MOVIE_H; i++)
	{
		in_rows[i] = viewport->movie_data + 
			...
			MOVIE_W * cmodel_calculate_pixelsize(TEMP_CMODEL) * i;
...
{
	if(viewport->do_movie) texture_iterate(viewport);
...
void init_movie(viewport_t *viewport)
{
	...
	viewport->frame_size = MOVIE_W * MOVIE_H * cmodel_calculate_pixelsize(TEMP_CMODEL);
...
//	if(viewport->total_frames > TOTAL_FRAMES) viewport->total_frames = TOTAL_FRAMES;
	...
	row_pointers = (unsigned char**)calloc(1, sizeof(unsigned char*) * TEXTURE_SIZE);
...
int main()
{
   ...
   glMaterialfv(GL_FRONT, GL_SPECULAR, diffuse_light);
